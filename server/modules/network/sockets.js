...
function close(socket) {
  ...
  if (index != -1) {
    ...
    socket.log(
      ...
      } disconnected! Views: ${views.length}. Clients: ${clients.length}.`
    );
    util.remove(players, index);
    ipCount.delete(socket.ip);
  } else {
    socket.log(
      `disconnected before entering the game. Views: ${views.length}. Clients: ${clients.length}.`
    );
  }
}
// Being kicked
...
function incoming(message, socket) {
  ...
  switch (m.shift()) {
    ...
    case "s":
      ...
      // Get data
      let name = m[0]
        .replace(Config.BANNED_CHARACTERS_REGEX, "")
        .replace(/§/g, "§§§§");
      let needsRoom = m[1];
      let autoLVLup = m[2];
      // Verify it
    ...
    case "C":
      ...
      if (
        player.body != null &&
        target.x ** 2 + target.y ** 2 > (player.body.fov * 1.25) ** 2
      ) {
    ...
    case "M":
      ...
      if (
        Config.SANITIZE_CHAT_MESSAGE_COLORS &&
        !socket.permissions.bypassChatSanitizer
      ) {
...
function traffic(socket) {
  ...
  return () => {
    // Kick if it's d/c'd
    if (
      performance.now() - socket.status.lastHeartbeat >
...
function flatten(data) {
  ...
  if (data.type & 0x01) {
    ...
    if (data.type & 0x04) {
      output.push(/* 21 */ data.name, /* 22 */ data.score);
    }
  }
...
class View {
  ...
  gazeUpon() {
    ...
    if (
      camera.lastUpdate - this.lastVisibleUpdate >
      Config.visibleListInterval
    ) {
      // Update our timer
