...
const socketInit = (port) => {
  ...
  socket.onopen = function socketOpen() {
    socket.open = true;
    global.message =
      "This server is restarting. Please try again in 5 seconds!";
  ...
  socket.onmessage = async function socketMessage(message) {
    ...
    switch (m.shift()) {
      ...
      case "svInfo": // For debugging.
        ...
        if (global.showDebug)
          console.log(
            `mspt: ${global.mspt} total entities on screen: ${global.entities.length} Player X: ${global.player.renderx.toFixed(1)} Player Y: ${global.player.rendery.toFixed(1)}`,
          );
        break;
